...
import apexSearch from "@salesforce/apex/GauExpendituresManager.searchActiveGeneralAccountingUnitsLikeName";
...
export default class GauExpenditureRow extends LightningElement {
    ...
    handleSearch(event) {
        ...
        apexSearch(event.detail).then((results) => {
    ...
    recalculateAmount() {
        ...
        this.gauExpenditure.amount = parseFloat(
            (parseFloat(this.percent / 100) * parseFloat(this.parentAmount)).toFixed(2)
        );
    }
